...
public interface LoanApprovalRepository extends JpaRepository<LoanApproval, Long> {
    ...
    @Query("SELECT COUNT(la) FROM LoanApproval la WHERE la.loan.id = :loanId AND la.usable = true AND la.loanStatus = ke.co.myfuture.Myfuture.Treasury.Loan.LoanStatus.APPROVED")
    int findApprovalsCount(@Param("loanId") Long loanId);


    @Query("SELECT CASE WHEN COUNT(la) > 0 THEN true ELSE false END FROM LoanApproval la WHERE la.loan.id = :loanId AND la.usable = true AND la.loanStatus = ke.co.myfuture.Myfuture.Treasury.Loan.LoanStatus.REJECTED")

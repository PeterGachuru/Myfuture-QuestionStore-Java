...
public class SavingWithdrawalValidator {
    @Autowired
    ...
    public boolean attachOtherTranEntries(Transaction transaction) {
        ...
        for (TranEntry tranEntry: transaction.getTranEntries()) {
            ...
            if (account.get().getBalance() < totalAmount) {
                System.out.println("No enough balance");
                return false;
            }
        }
        return validatorCommons.addCashAccount(totalAmount, TranType.CREDIT, transaction, transaction.getContributionsPlan().getName()+" withdrawal ");
    }
}

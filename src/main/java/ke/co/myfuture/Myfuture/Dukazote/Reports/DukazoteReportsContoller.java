package ke.co.myfuture.Myfuture.Dukazote.Reports;
...
@RestController
@RequestMapping("dukazote/reports")
@CrossOrigin("*")
@Slf4j
public class DukazoteReportsContoller {
    @Value("${report.logo}")
    String logo;
    ...
    @Value("${datasource.dukazote.url}")
    private String db;
    @Value("${datasource.dukazote.username}")
    private String username;
    @Value("${datasource.dukazote.password}")
    private String password;


    @GetMapping("/load")
    public ResponseEntity<?> loadPdf(HttpServletRequest request, @RequestParam Long cartId) {
...
//            if (!loansList.isEmpty()){
            ...
            JasperReport compilereport = JasperCompileManager.compileReport(new FileInputStream(path + "/"+"cart_receipt.jrxml"));

            Map<String, Object> parameters = setParameters(cartId);
...
//            }
    ...
    private Map<String, Object> setParameters(Long cartId) {
        Map<String, Object> parameters = new HashMap<>();
        parameters.put("logo", logo);
        parameters.put("business_name", appname);

...
public interface WritersbroadcastRepository extends JpaRepository<Writersbroadcast, Long> {
    ...
    @Query(value = "SELECT * FROM writersbroadcast WHERE targets = 'pupils' AND date_sent IS NULL", nativeQuery = true)
    List<Writersbroadcast> findToSendForPupils();
    @Query(value = "SELECT * FROM writersbroadcast WHERE targets = 'writers' AND date_sent IS NULL", nativeQuery = true)
